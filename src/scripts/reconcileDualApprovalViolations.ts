 ...
 * 
 * Run with: npm run reconcile-dual-approval
 * 
 * Options (set via environment variables):
 *   DRY_RUN=false    Actually apply the changes (default: true = preview only)
 *   ORG_FILTER=mgb   Filter to a specific organization
 */
...
const orgFilter = process.env.ORG_FILTER || null;
...
// ZAR and LSL are pegged 1:1 in the Common Monetary Area
const EXCHANGE_RATES: Record<string, Record<string, number>> = {
  'USD': { 'XOF': 603.48, 'LSL': 18.49, 'ZMW': 27.5, 'ZAR': 18.49 },
  'XOF': { 'USD': 0.00166, 'LSL': 0.0306, 'ZMW': 0.0456, 'ZAR': 0.0306 },
  'LSL': { 'USD': 0.054, 'XOF': 32.68, 'ZMW': 1.49, 'ZAR': 1.0 },
  'ZMW': { 'USD': 0.036, 'XOF': 21.94, 'LSL': 0.67, 'ZAR': 0.67 },
  'EUR': { 'USD': 1.08, 'XOF': 655.96, 'LSL': 19.97, 'ZMW': 29.7, 'ZAR': 19.97 },
  'ZAR': { 'USD': 0.054, 'XOF': 32.68, 'LSL': 1.0, 'ZMW': 1.49 },
};
...
async function main() {
  ...
  try {
    ...
    // Load service account JSON using fs (ESM compatible)
    ...
    console.error('Make sure you have the service account JSON file or GOOGLE_APPLICATION_CREDENTIALS set.');
    console.error(error);
    process.exit(1);
  }
  ...
  const rulesSnapshot = await db.collection('referenceData_rules').get();
  
  const orgRules: Record<string, { rule3: RuleData; rule5: number }> = {};
  
  for (const docSnap of rulesSnapshot.docs) {
  ...
  // Step 2: Load all PRs in PO statuses (APPROVED, ORDERED, COMPLETED)
  // These are PRs that have passed approval and should have had dual approval verified
  const poStatuses = ['APPROVED', 'ORDERED', 'COMPLETED'];
  console.log(`Step 2: Loading PRs in PO statuses (${poStatuses.join(', ')})...`);
  ...
  for (const status of poStatuses) {
    const snapshot = await db.collection('purchaseRequests')
      .where('status', '==', status)
      .get();
    console.log(`  ${status}: ${snapshot.size} PRs`);
    allDocs.push(...snapshot.docs);
  }
  
  console.log(`  Total: ${allDocs.length} PRs in PO statuses\n`);
  ...
  for (const docSnap of allDocs) {
    const data = docSnap.data();
    const prNumber = data.prNumber || docSnap.id;
  ...
  if (isDryRun) {
    ...
    console.log('To apply changes, run with: DRY_RUN=false npm run reconcile-dual-approval');
    process.exit(0);
  }
  ...
  for (const v of violations) {
    try {
      ...
      if (!currentData) {
        console.error(`  ✗ Could not fetch current data for ${v.prNumber}`);
        errorCount++;
        continue;
      }
