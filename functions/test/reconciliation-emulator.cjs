...
(async()=>{
 ...
 const a={name:'Legacy clamp',description:'ABC bundled conductor clamp 25-95mm2',unit_of_measure:'EA',item_class:'Inventory',status:'Available',country_id:'1',organization_id:'1pwr_lesotho',quantity:25};
 ...
 const body={eventId:'event1',ugpPartId:'abc-dead-end-clamp',assetIds:['a','b'],decision:'same_part',confirmedPartId:'abc-dead-end-clamp',expectedAssets:{a:identity(a),b:identity({...a,name:'Other spelling'})},evidence:'Manufacturer datasheet and physical item were checked.',retParticipant:'RET reviewer',retVerified:true,amVerified:true,currentSpecificationVerified:true};
 ...
 await assert.rejects(fn.run({...body,confirmedPartId:'earthing-rod'},ctx),/exact UGP requirement/);
 const result=await fn.run(body,ctx);assert.equal(result.published,true);
 const saved=(await db.doc('am_core_assets/a').get()).data();assert.equal(saved.quantity,25);assert.equal(saved.description,a.description);assert.equal(saved.original_catalogue_identity.name,'Legacy clamp');assert.equal(saved.original_catalogue_identity.description,a.description);assert.equal(saved.name,'ABC Dead-End Clamp');assert.equal(saved.canonical_part_number,'abc-dead-end-clamp');assert.deepEqual(saved.catalogue_aliases,['Legacy clamp']);
 ...
 await db.doc('am_core_assets/d').set({name:'Legacy clamp',description:'ABC bundled conductor clamp 25-95mm2',unit_of_measure:'EA',item_class:'Inventory',status:'Available',country_id:'1',quantity:25});
 ...
 const cross={...a,name:'Steel cross arm',description:'2.5m galvanized steel cross arm'};await db.doc('am_core_assets/c').set(cross);
 assert.equal((await fn.run({...body,eventId:'decimal',ugpPartId:'cross-arm-mv-2.5m',confirmedPartId:'cross-arm-mv-2.5m',assetIds:['c'],expectedAssets:{c:identity(cross)}},ctx)).published,true);
 const wrong={...a,name:'Earth clamp',description:'Rod to wire clamp'};await db.doc('am_core_assets/lKph3eLgS9wnxkJyTomK').set(wrong);
 await assert.rejects(fn.run({...body,eventId:'known-wrong',assetIds:['lKph3eLgS9wnxkJyTomK'],expectedAssets:{lKph3eLgS9wnxkJyTomK:identity(wrong)}},ctx),/not ABC/);
 assert.equal((await fn.run({...body,eventId:'known-component',decision:'component',ownerName:'RET lead',dueDate:'2026-10-01',assetIds:['lKph3eLgS9wnxkJyTomK'],expectedAssets:{lKph3eLgS9wnxkJyTomK:identity(wrong)}},ctx)).published,false);
