...
async function updateMatchedUser(
  ...
  const nexusPatch: Record<string, unknown> = { updatedAt: now };
  if (patch.email) nexusPatch.email = patch.email;
  if (patch.firstName) nexusPatch.firstName = patch.firstName;
  if (patch.lastName) nexusPatch.lastName = patch.lastName;
  ...
  if (opts.detectDepartures !== false) {
    ...
    for (const [hrId, { uid, data }] of prByHrId) {
      if (!hrIds.has(hrId)) {
        ...
        try {
          const departurePatch: Record<string, unknown> = { hrStatus: "inactive", hrSyncedAt: now, updatedAt: now };
          if (allowDeactivation) {
