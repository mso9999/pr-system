/** Allowlisted purchasing evidence. Quantities are not AM receipts or stock. */
...
export type LineDataStatus = "available" | "empty" | "missing" | "invalid";

export function planningText(value: unknown): string | null {
...
export function projectLines(value: unknown): { items: ProcurementLine[]; status: LineDataStatus } {
  if (value == null) return { items: [], status: "missing" };
  if (!Array.isArray(value)) return { items: [], status: "invalid" };
  ...
  return {
    ...
    items: value.map((raw, sourceIndex) => {
      ...
      const issues: string[] = [];
      if (!validRow) issues.push("invalid_line");
      if (!id) issues.push("line_id_missing");
      else if (ids.get(id)! > 1) issues.push("line_id_duplicate");
      if (qty === null) issues.push("quantity_unknown_or_invalid");
      if (!uom) issues.push("uom_missing");
