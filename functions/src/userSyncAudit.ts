...
interface AuditResult {
  ...
  totals: {
    ...
    authOrphans: number;
    firestoreOrphans: number;
    emailMismatch: number;
    anonymousAuthIgnored: number;
    properlySynced: number;
  };
  authOrphans: { uid: string; email: string | null; displayName: string | null; lastSignIn: string | null; createdAt: string | null }[];
  firestoreOrphans: { uid: string; email: string; firstName: string; lastName: string }[];
  emailMismatch: { uid: string; firestoreEmail: string; authEmail: string }[];
}
...
function isAnonymousAuth(u: admin.auth.UserRecord): boolean {
...
export async function runAudit(): Promise<AuditResult> {
  ...
  const fsByUid = new Map<string, FirebaseFirestore.DocumentData>();
  ...
  for (const u of authUsers) authByUid.set(u.uid, u);
...
function htmlSummary(r: AuditResult): string {
...
export const weeklyUserSyncAudit = functions
...
export const runUserSyncAudit = functions.https.onCall(async (_data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Sign-in required");
  }
  const callerDoc = await db.collection("users").doc(context.auth.uid).get();
  const lvl = Number(callerDoc.data()?.permissionLevel);
