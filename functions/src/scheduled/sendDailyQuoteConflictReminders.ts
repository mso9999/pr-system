    ...
    try {
      ...
      const emailPromises = conflictedPRsSnapshot.docs.map(async (prDoc) => {
        ...
        try {
          ...
          const approver1Email = approver1Doc.exists ? approver1Doc.data()?.email : null;
          const approver2Email = approver2Doc.exists ? approver2Doc.data()?.email : null;
          ...
          const approver1Name = approver1Doc.exists
            ? `${approver1Doc.data()?.firstName} ${approver1Doc.data()?.lastName}`
            : 'Approver 1';
          const approver2Name = approver2Doc.exists
            ? `${approver2Doc.data()?.firstName} ${approver2Doc.data()?.lastName}`
            : 'Approver 2';
          ...
          const firstQuote = quotes.find((q: any) => q.id === firstQuoteId);
          const secondQuote = quotes.find((q: any) => q.id === secondQuoteId);

          // Calculate days in conflict
          const conflictStartDate = new Date(pr.approvalWorkflow?.lastUpdated || pr.updatedAt);
          const daysInConflict = Math.floor(
            (Date.now() - conflictStartDate.getTime()) / (1000 * 60 * 60 * 24)
          );
...
<table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
  ...
  <tr>
    ...
    <td style="border: 1px solid #ddd; padding: 12px; text-align: right;">${firstQuote?.amount?.toFixed(2) || 'N/A'} ${firstQuote?.currency || ''}</td>
  </tr>
  <tr>
    ...
    <td style="border: 1px solid #ddd; padding: 12px; text-align: right;">${secondQuote?.amount?.toFixed(2) || 'N/A'} ${secondQuote?.currency || ''}</td>
  </tr>
</table>
...
- ${approver1Name}: ${firstQuote?.vendorName || 'Unknown'} - ${firstQuote?.amount?.toFixed(2) || 'N/A'} ${firstQuote?.currency || ''}
- ${approver2Name}: ${secondQuote?.vendorName || 'Unknown'} - ${secondQuote?.amount?.toFixed(2) || 'N/A'} ${secondQuote?.currency || ''}
